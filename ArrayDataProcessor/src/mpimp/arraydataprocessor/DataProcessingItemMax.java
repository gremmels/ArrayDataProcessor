...
public class DataProcessingItemMax extends AbstractDataProcessingItem {

	public DataProcessingItemMax(String columnName) {
		super(columnName);
	...
	public void processData() {
		...
		for (String key : keys) {
			...
			// calculate the maximum value from all values retrieved for
			// the key and the specified column
			if (values.size() > 0) {
				resultColumn_.put(key, Collections.max(values));
			} else {
