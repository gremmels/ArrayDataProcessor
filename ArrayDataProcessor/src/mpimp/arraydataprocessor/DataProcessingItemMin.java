...
public class DataProcessingItemMin extends AbstractDataProcessingItem {

	public DataProcessingItemMin(String columnName) {
		super(columnName);
	...
	public void processData() {
		...
		for (String key : keys) {
			...
			// calculate the minimum value from all values retrieved for
			// the key and the specified column
			if (values.size() > 0) {
				resultColumn_.put(key, Collections.min(values));
			} else {
